...
def load_dataset():
    ...
    # Default fallback demo distribution
...
# ---------------------------------------------------------
with st.sidebar:
    st.image("https://images.unsplash.com/photo-1497435334941-8c899ee9e8e9?w=500&q=80", use_column_width=True)
    ...
    st.caption("Baseline ranges for green technology evaluation:")
    st.markdown(
        "- **Carbon Emissions:** gCO2/kWh\n"
        "- **Energy Output:** MWh produced\n"
        "- **Renewability Index:** 0.0 (Fossil) to 1.0 (Clean)\n"
...
with tab_predict:
    ...
    with col_input:
        carbon = st.slider(
            "Carbon Emissions (gCO2/kWh)",
            min_value=40.0, max_value=450.0,
            value=float(df_raw['carbon_emissions'].median()),
            step=1.0,
            help="Direct carbon footprint of generation."
        )
        energy = st.slider(
            "Energy Output (MWh)",
            min_value=50.0, max_value=1100.0,
            value=float(df_raw['energy_output'].median()),
            step=5.0,
            help="Total operational energy generated."
        )
        renew = st.slider(
            "Renewability Index",
            min_value=0.0, max_value=1.0,
            value=float(df_raw['renewability_index'].median()),
            step=0.01,
            help="Fraction of cycle driven by renewable power sources."
        )
        cost = st.slider(
            "Cost Efficiency Index",
            min_value=0.4, max_value=5.0,
            value=float(df_raw['cost_efficiency'].median()),
            step=0.05,
            help="Normalized economic overhead score (lower is more cost-effective)."
        )
...
with tab_batch:
    ...
    st.write("Upload a CSV file containing columns: `carbon_emissions`, `energy_output`, `renewability_index`, `cost_efficiency`.")
    ...
    if uploaded_file is not None:
        ...
        if missing_cols:
            ...
            st.download_button(
                "⬇️ Download Evaluated CSV",
                data=csv_data,
                file_name="greentech_predictions.csv",
                mime="text/csv"
            )
    else:
...
with tab_about:
    ...
    - **Negative Weights (Reduces Sustainability):** High `carbon_emissions` and elevated `cost_efficiency` values drive probabilities toward non-sustainable status.
    - **Positive Weights (Increases Sustainability):** Elevated `renewability_index` strongly favors sustainable classification.
    """)
